...
// b. Dividing the spectrum into angular sectors
//Command line arguments: ./executable input.raw output.text ROWS COLS 64
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#define PI 3.1415926
...
// Parameters: K          - Number of angular sectors
//             N          – Rows/Columns of the original image
//             DFT[][]    – Fourier transform image
//             fout       - FILE pointer that points to output text file
void FourierSpectrumAnalysis(int K, int N, COMPLEX DFT[N][N], FILE *fout)
{
  int distance, index;
  double AH[K];
  float m, D_Theta = 360 / K, Theta;
  
  // Initialize the angular histogram array
  for(int i=0; i<K; i++)
  {
    AH[i] = 0.0;
  }
  ...
  for(int i=0; i<N; i++)
  {
    for(int j=0; j<N; j++)
    {
        // Calculates the slope between the current pixel(i, j) and center pixel (N/2, N/2)
        m = (float) (j - N/2) / (i - N/2);
        
        // Angle wrt the line i=N/2 is calculated
        Theta = atan(m) * 180 / PI;
        
        // Modifications are done to Theta so that Theta lies between 0 degree and 360 degrees
          if(i < N/2)
          {
            Theta = 90 + Theta;
          }
        ...
        // Calculates the angular histogram index
        index = (int) Theta / D_Theta;
        
        // Sets the index value to 'K' if the index exceeds the value 'K'
        if(index > K)
        {
          index = K;
        }

        // Spectra value of corresponding pixel is calculated and is added to Angular Histogram
        // array of corresponding index
        AH[index-1] += sqrt(DFT[i][j].r * DFT[i][j].r + DFT[i][j].i * DFT[i][j].i); 
    }    
  }
  ...
  for(int i=0; i<K; i++)
  {
    // Writes the Angular Histogram index and array values to output text file
    fprintf(fout, "%d,%f \n", i, AH[i]);
  } 
...
//             DFT[][]    - Fourier transform image
//             fout       – FILE pointer that points to output text file
void TwoDimensionsalFFT(int K, int M, int N, unsigned char img[M][N], COMPLEX C_img[M][N], COMPLEX DFT[M][N], FILE *fout)
{
 ...
 // Call FourierSpectrumAnalysis function to calculate the Angular histogram
 FourierSpectrumAnalysis(K, N, DFT, fout);
}
...
int main(int argc, char **argv)
{
 ...
 // arg 5 is the number of angular sectors
 ...
 TwoDimensionsalFFT(k, x, y, in_buf, C_img, DFT, fout);
