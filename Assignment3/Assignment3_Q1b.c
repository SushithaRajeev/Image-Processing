...
void get_normalized_parameters(int *mean, int ROW1, int COL1, unsigned char img[ROW1][COL1], int startIndex1, int endIndex1, int startIndex2, int endIndex2)
{
    int sumOfMean = 0, sumOfStdDeviation = 0;

    for(int p=startIndex1; p<startIndex2; p++)
    {
        for(int q=endIndex1; q<endIndex2; q++)
        {
            sumOfMean =sumOfMean + img[p][q];
        }
    }

    *mean = sumOfMean / ((startIndex2 - startIndex1) * (endIndex2 - endIndex1 ));

    for(int x=startIndex1; x<startIndex2; x++)
    {
        for(int y=endIndex1; y<endIndex2; y++)
        {
            sumOfStdDeviation =  sumOfStdDeviation + abs(img[x][y] - *mean) * abs(img[x][y] - *mean);
        }
    }
}

// correlation and template matching
void normalized_correlation(int N1, int M1, int N2, int M2, unsigned char img1[M1][N1], unsigned char img2[M2][N2], unsigned char imgNew1[M1][N1])
{   
    int i, j, letterRMean, originalImageMean, originalImageStandardDeviation;
    ...
    // get the parameters of letter-R image used for normalization
    get_normalized_parameters(&letterRMean, M2, N2, img2, 0, 0, M2, N2);
    ...
    for (i = 0; i < M1 - M2; i++){
        for(j = 0; j < N1 - N2; j++){  
            for(int x = 0; x < M2; x++){
                ...
                // get the parameters of original image used for normalization
                get_normalized_parameters(&originalImageMean, M1, N1, img1, i, j, i+M2, j+N2);

                for(int y = 0; y < N2; y++){
                    new[i][j] += (img1[i+x][j+y] - originalImageMean)*(img2[x][y] - letterRMean);
                    normalize1[i][j] += (img1[i+x][j+y] - originalImageMean) * (img1[i+x][j+y] - originalImageMean);
                    normalize2[i][j] += (img2[x][y] - letterRMean) * (img2[x][y] - letterRMean);
                }
            }
        }        
    }
...
int main(int argc, char **argv)
{
    ...
    normalized_correlation(COLS1, ROWS1, COLS2, ROWS2, in_buf1, in_buf2, out_buf1);
